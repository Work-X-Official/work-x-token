  ...
  describe("Testing getClaimable", async () => {
    ...
    describe("getClaimable with multiple nfts", async () => {
      ...
      it("Go to month 1, getClaimable returns for each the reward of month 0", async () => {
        await mineDays(30, network);
        expect(await reward.getClaimable(nftId1)).to.equal(amount(REWARDS[0]).mul(shares1).div(totalShares));
        expect(await reward.getClaimable(nftId2)).to.equal(amount(REWARDS[0]).mul(shares2).div(totalShares));
        expect(await reward.getClaimable(nftId3)).to.equal(amount(REWARDS[0]).mul(shares3).div(totalShares));
      });
